#!/usr/bin/env python3
"""
NOWPayments signup clipboard filler.
Fields: email, password (2x), CAPTCHA (manual), then navigate to API keys.
"""
import subprocess
import sys
...
FIELDS = [
    ('section', '═══ NOWPAYMENTS SIGNUP ═══', None),
    ('Email', 'sichermayor@deltajohnsons.com', None),
    ('Password', 'BeastPay_NP_2026!Secure', None),
    ('Confirm Password', 'BeastPay_NP_2026!Secure', None),
    ('section', '═══ AFTER SIGNUP ═══', None),
    ('Navigate to', 'https://nowpayments.io/dashboard/auth/api-keys', None),
    ('section', '═══ COPY API KEY ═══', None),
    ('API Key location', 'Dashboard → Settings → API Keys (click "Show")', None),
]
...
def main():
    ...
    print(f'{C}{BOLD}  🚀  NOWPAYMENTS SIGNUP HELPER{RESET}')
    print(f'{C}{BOLD}{"=" * 60}{RESET}')
    print()
    print(f'{Y}STEPS:{RESET}')
    print(f'   1. Open https://nowpayments.io/signup in browser')
    ...
    print(f'   6. Solve CAPTCHA manually, then continue')
    print(f'   7. After signup, navigate to API Keys and copy key')
    print()
